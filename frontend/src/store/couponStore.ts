...
export const useCouponStore = create<CouponState>((set) => ({
  ...
  fetchCoupons: async () => {
    ...
    try {
      ...
      // Filter out coupons already used by the user so they are not displayed
      const available = Array.isArray(data) ? data.filter(c => !c.is_used) : [];
      set({ coupons: available, loading: false });
