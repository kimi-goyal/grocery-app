...
export default function CouponsPage() {
  const { coupons, addCoupon, toggleActive, deleteCoupon } = useCouponStore();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ code: '', discount: '', type: 'percentage' as 'percentage' | 'flat', minOrder: '', maxDiscount: '', usageLimit: '', expiry: '', description: '', active: true });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 2. Optimistically insert into couponStore
    // 3. Show success toast
    addCoupon({
      code: form.code.toUpperCase(), discount: Number(form.discount),
      type: form.type, minOrder: Number(form.minOrder),
      maxDiscount: Number(form.maxDiscount), usageLimit: Number(form.usageLimit),
      expiry: form.expiry, description: form.description, active: form.active,
    });
    setOpen(false);
    setForm({ code: '', discount: '', type: 'percentage', minOrder: '', maxDiscount: '', usageLimit: '', expiry: '', description: '', active: true });
  };
    ...
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 animate-fadeUp delay-100">
          {coupons.map(c => {
            const usagePct = Math.min((c.usedCount / c.usageLimit) * 100, 100);
            return (
