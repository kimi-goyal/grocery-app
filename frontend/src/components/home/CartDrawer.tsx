...
export default function CartDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { coupons: userCoupons, loading: loadingCoupons, fetchCoupons } = useCouponStore();
  ...
  useEffect(() => {
    ...
    fetchCoupons().catch(() => {});
  }, [open, items.length, fetchCoupons]);
              ...
              <button
                onClick={() => {
                  ...
                  if (firstCategoryName) {
                    navigate(`/home?category=${encodeURIComponent(firstCategoryName)}`);
                  } else {
                    navigate('/home');
                  }
                }}
                className="btn-primary glow-pink-sm mt-2"
