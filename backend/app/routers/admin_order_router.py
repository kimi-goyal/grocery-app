...
async def update_order_status(
    ...
    # Auto-assign driver when order status changes to "On the Way"
    if data.status == OrderStatus.on_the_way and not order.driver_id:
        try:
            driver_service.auto_assign_driver_on_status_change(db, order)
        except HTTPException as e:
            # If no drivers available, still update status but log warning
            print(f"Warning: Could not auto-assign driver to order {order.order_number}: {e.detail}")
    ...
    return {"msg": "Updated", "driver_assigned": order.driver_id is not None}